...
public class PaletteLoader {
		...
		case FE6:
			for (FE6Data.Character character : FE6Data.Character.allPlayableCharacters) {
				...
				for (PaletteInfo paletteInfo : FE6Data.Palette.palettesForCharacter(charID)) {
					...
					if(verbose) { DebugPrinter.log(DebugPrinter.Key.PALETTE, "Initializing Character 0x" + Integer.toHexString(charID) + " (" + fe6char.toString() + ")" + " with palette at offset 0x" + Long.toHexString(paletteInfo.getOffset()) + " (Class: " + Integer.toHexString(classID) + " (" + fe6class.toString() + "))"); }
				}
			}
			for (FE6Data.Character boss : FE6Data.Character.allBossCharacters) {
				...
				for (PaletteInfo paletteInfo : FE6Data.Palette.palettesForCharacter(charID)) {
					...
					referenceMap.put(classID, new PaletteV2(handler, paletteInfo));
					FE6Data.Character fe6char = FE6Data.Character.valueOf(charID);
					if(verbose) { DebugPrinter.log(DebugPrinter.Key.PALETTE, "Initializing Boss 0x" + Integer.toHexString(charID) + " (" + fe6char.toString() + ")" + " with palette at offset 0x" + Long.toHexString(paletteInfo.getOffset())); }
				}
			}
		...
		case FE7:
			for (FE7Data.Character character : FE7Data.Character.allPlayableCharacters) {
				...
				for (PaletteInfo paletteInfo : FE7Data.Palette.palettesForCharacter(charID)) {
					...
					if(verbose) { DebugPrinter.log(DebugPrinter.Key.PALETTE, "Initializing Character 0x" + Integer.toHexString(charID) + " (" + fe7char.toString() + ")" + " with palette at offset 0x" + Long.toHexString(paletteInfo.getOffset()) + " (Class: " + Integer.toHexString(classID) + " (" + fe7class.toString() + "))"); }
				}
			}
			for (FE7Data.Character boss : FE7Data.Character.allBossCharacters) {
				...
				for (PaletteInfo paletteInfo : FE7Data.Palette.palettesForCharacter(charID)) {
					...
					referenceMap.put(classID, new PaletteV2(handler, paletteInfo));
					FE7Data.Character fe7char = FE7Data.Character.valueOf(charID);
					if(verbose) { DebugPrinter.log(DebugPrinter.Key.PALETTE, "Initializing Boss 0x" + Integer.toHexString(charID) + " (" + fe7char.toString() + ")" + " with palette at offset 0x" + Long.toHexString(paletteInfo.getOffset())); }
				}
			}
	...
	public void adaptFE8CharacterToClass(GBAFECharacterData character, GBAFECharacterData reference, int newClassID, Boolean isBoss) {
		...
		if(verbose) { DebugPrinter.log(DebugPrinter.Key.PALETTE, "Adapting Character " + FE8Data.Character.valueOf(characterID).toString() + " to class " + FE8Data.CharacterClass.valueOf(newClassID).toString() + " using Reference " + FE8Data.Character.valueOf(referenceID).toString()); }
		...
		if (willBecomeTrainee) {
			if (fe8Mapper.classIDMappedToCharacterForType(charID, SlotType.TRAINEE) == newClassID && !character.hasBattlePaletteOverrides()) {
				if(verbose) { DebugPrinter.log(DebugPrinter.Key.PALETTE, "Same trainee class found. Skipping palette replacement."); }
			} else {
			...
			if (fe8Mapper.classIDMappedToCharacterForType(charID, SlotType.PRIMARY_BASE) == newClassID && !character.hasBattlePaletteOverrides()) {
				if(verbose) { DebugPrinter.log(DebugPrinter.Key.PALETTE, "Same base class found. Skipping palette replacement."); }
			} else {
			...
			if (fe8Mapper.classIDMappedToCharacterForType(charID, SlotType.FIRST_PROMOTION) == newClassID && !character.hasBattlePaletteOverrides()) {
				if(verbose) { DebugPrinter.log(DebugPrinter.Key.PALETTE, "Same Promoted class found. Skipping palette replacement."); }
			} else {
	...
	private void integrateFE8PaletteIfPossible(int charID, PaletteV2 adaptedPalette, SlotType type) {
		...
		if (paletteID != 0) {
			...
			if (offset != null) {
				...
				if(verbose) { DebugPrinter.log(DebugPrinter.Key.PALETTE, "Assigned New Palette ID 0x" + Integer.toHexString(paletteID).toUpperCase() + " (length: " + Integer.toString(newData.length) + ")"); }
	...
	private PaletteV2 v2PaletteForClass(int newClassID, PaletteV2[] referencePalettes, PaletteV2.PaletteType type, PaletteColor[] supplementalHairColors) {
		...
		if (template == null) 
		{
			if(verbose) { DebugPrinter.log(DebugPrinter.Key.PALETTE, String.format("No Template Palette found for Class %d.", newClassID)); }
		}
		PaletteV2 adapted = new PaletteV2(template);
		if(verbose) { DebugPrinter.log(DebugPrinter.Key.PALETTE, "Adapting Palette using " + referencePalettes.length + " reference palettes."); }
	...
	public void enqueueChange(GBAFECharacterData character, GBAFECharacterData reference, CharacterDataLoader charData, ClassDataLoader classData, Integer targetClassID, boolean needsPromotion) {
		...
		if(verbose) { DebugPrinter.log(DebugPrinter.Key.PALETTE, "Enqueuing change for character " + charData.debugStringForCharacter(characterID) + " to class " + classData.debugStringForClass(targetClassID) + " using reference " + charData.debugStringForCharacter(referenceID)); }
		...
		if (isPromoted) {
			...
			if (originalPalette != null && originalPalette.getClassID() == targetClassID && !character.hasBattlePaletteOverrides()) {
				if(verbose) { DebugPrinter.log(DebugPrinter.Key.PALETTE, "Same promoted class found. Skipping adapting palette."); }
			} else {
			...
			PaletteV2 originalPalette = paletteByPaletteIDV2.get(originalPaletteIndex);
			if (originalPalette != null && originalPalette.getClassID() == targetClassID && !character.hasBattlePaletteOverrides()) {
				if(verbose) { DebugPrinter.log(DebugPrinter.Key.PALETTE, "Same unpromoted class found. Skipping adapting palette."); }
			} else {
	...
	public void recordUpdatedPalettes(RecordKeeper rk, CharacterDataLoader charData, ClassDataLoader classData, TextLoader textData) {
		...
		for (Change change : queuedChanges) {
			...
			String characterName = textData.getStringAtIndex(character.getNameIndex(), true);
			if(verbose) { DebugPrinter.log(DebugPrinter.Key.PALETTE, "Recording updated palette for " + characterName); }
			
			if (change.basePalette != null) {
				if(verbose) { DebugPrinter.log(DebugPrinter.Key.PALETTE, "Base Palette Recorded"); }
			...
			if (change.promotedPalette != null) {
				if(verbose) { DebugPrinter.log(DebugPrinter.Key.PALETTE, "Promoted Palette Recorded"); }
